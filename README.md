...
| 查询 | 航班/票价、价格构成、目的地天气（Open-Meteo 真实预报）、延误预测、价格趋势、**行李额度**（按航司/舱位的免费托运与超重费率，含会员等级加成）、**联网搜索**（博查：景点门票/活动/交通等实时信息）、订单账单、投诉记录 |
...
- **确认卡片（HITL，服务端强制）**：订票/退票/改签/值机等写操作，LLM 只负责收集参数与解释，
  通过伪工具产生确认卡片；用户确认后由 REST 接口在代码层执行——**写库路径不经过 LLM**。
  服务端用**一次性确认凭证**（`services/confirm_token.py`）校验"用户确实确认过"：
  准备接口（报价/座位图）或聊天卡片签发 `confirm_token`（绑定会员+动作+目标+参数指纹，15 分钟有效、
  用完即废），写接口缺失/过期/不匹配一律 **403**。没有凭证裸调写接口是行不通的。
- **退款幂等**：`POST /api/refund` 带唯一 `requestId` 作幂等键（`refunds` 流水表），
  同一请求重放只退一次；订单状态流转带 `AND status IN (...)` 守卫，杜绝重复退款。
- **审计日志**：越权尝试、高危输入拦截、每笔写操作与幂等命中都落 `audit_logs`，
  管理端 `GET /admin/api/audits` 可查可统计（`stats` 含越权尝试拦截数）。
- **工具层硬安全**：登录身份经 ContextVar 受信通道注入工具执行上下文，
  仓库层**从订单反查归属**再校验（不再信任调用方传入的 member_id，传 None 也不放行）；
  后台任务/支付回调等无身份场景用 `security.system_context()` 显式声明。
...
| 层级 | 命令 | 耗时 | Token | 何时跑 |
|---|---|---|---|---|
| 单元测试 | `python -m pytest test_unit.py -q` | ~15秒 | **0** | **每次改代码后随便跑**（费率/状态机/注册/越权/通知/搜索解析/标题/趋势聚合/值机选座/行李额度/JWT/支付幂等/评估判定 等 169 用例，独立临时库） |
| 任务集评估 | `python eval/run_eval.py --go` | ~15分钟 | **高**（50 轮真实对话） | 里程碑/答辩前采集指标；先跑 `--dry-run` 零成本校验 |
...
> **离线评估集**见 [`eval/README.md`](eval/README.md)：50 条任务覆盖查询/改签/退票/选座/行李/投诉六类，
> 产出端到端完成率与工具调用准确率（该调没调 / 调错工具 / 参数错三分类），
> 另有 6 条安全探针（提示词注入 + 越权）对应答辩演示桥段；基线与对比表见 [`eval/BASELINE.md`](eval/BASELINE.md)。
...
│   └── checkin_repo.py           #   值机选座/登机牌（座位图、值机窗口、原子占座）
├── eval/                         # 离线评估集：50 条任务 + 6 条安全探针 + 执行器 + 基线
