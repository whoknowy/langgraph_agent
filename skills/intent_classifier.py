...
AGENT_OPTIONS = {
    "product": "机票业务：航班查询/比价、购票/订票/买票/下单出票、预订咨询、机票价格构成、目的地天气、航班延误预测、价格走势",
    "billing": "已有订单的处理：查订单、账单明细、支付、退款、改签、发票、值机选座、登机牌（**不含新购票/买票下单**，买票一律归 product）",
...
_SYSTEM_PROMPT = (
    ...
    "4. **买票/订票/购票/下单出票，以及对此类请求的确认（\"确认/就订这个\"）**，"
    "一律选 product——只有机票专员能发起购票，账单专员只处理已有订单；\n"
    "5. 只输出 JSON，不要输出任何其他文字：{\"agent\": \"product|billing|complaint|general\", \"note\": \"一句话依据\"}"
)
