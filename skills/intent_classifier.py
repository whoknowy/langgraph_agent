...
只做一件事——把用户这句话分给 5 个业务 Agent 中的一个。
...
# 5 个 Agent 的职责边界（与 agents/ 下 5 个实现一一对应）
AGENT_OPTIONS = {
    "product": "机票业务：航班查询/比价/预订咨询、机票价格构成、目的地天气、航班延误预测、价格走势、行李托运额度与超重费",
    "billing": "订单与账单：查订单、账单明细、支付、退款、改签费用、发票、值机选座、登机牌、订单对应的行李额度",
    "complaint": "投诉与不满：投诉查询/提交、索赔、要求道歉或赔偿、表达强烈不满",
    "trip_planner": "行程规划：帮忙安排旅行行程、做旅游攻略、规划几天几晚的玩法（含机票+景点+天气的综合规划）",
    "general": "问候闲聊、感谢、或与上述几类都无关的其他咨询",
}

_SYSTEM_PROMPT = (
    "你是航空客服系统的意图分类器。根据对话历史和用户最新消息，"
    "从下面五个客服专员中选择最合适的一个处理用户最新消息：\n"
...
def classify_intent(query: str, history: Optional[List[Dict]] = None,
    ...
    Args:
        ...
        session_id / member_id: 用于把这一次分类的 LLM 调用纳入 Langfuse
            trace（按会话分组回放）。未启用 Langfuse 时不影响行为。
