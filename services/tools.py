...
def submit_booking_request(flight_no: str, flight_date: str, cabin: str, passengers: int = 1) -> str:
    """【订票确认工具·必须调用】当航班号、出发日期、舱位、乘机人数四项信息齐全时，
    **必须调用本工具**来发起订票确认，没有例外。

    确认卡片只有调用本工具才会出现在用户页面上——绝不允许只在回复文字里说
    "请点击确认预订按钮"而不调用本工具（那样用户没有按钮可点，无法下单）。
    调用后向用户复述航班号/日期/舱位/人数与总价，引导其点击"确认预订"完成下单。
    参数不全时不要调用，先向用户追问缺失信息。
...
def change_request(order_no: str, new_flight_no: str, new_date: str, new_cabin: str) -> str:
    ...
    免改签费，票价差多退少补：确认卡片上会实时显示「需补差价 ¥X / 退回差价 ¥X」，
    **不要自己计算或报出差价金额**（卡片上的才是实时真实报价，报错了用户会误会）。
    补差价要在支付宝收银台付款成功后才改签生效。本工具不会真正改签。
