...
- **管理端**：<http://127.0.0.1:5000/admin> —— 登录管理员 `admin / admin123`（首次登录会强制要求设置新密码，之后用新密码登录）
...
4. 「帮我把订单 Oxxxxx 改签到下周三」→ 改签卡片（**卡片上直接显示差价**）→ 确认改签；
   若需补差价会跳到支付宝收银台，**付款成功后改签才生效**；退回差价则原路退回支付宝；
