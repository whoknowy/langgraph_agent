...
参数**，其余按键名 ASCII 升序以 `键=值` 用 `&` 连接，再用支付宝公钥做
SHA256withRSA。⚠️ 空值参数必须先剔除——下单时未设置 `body`，支付宝会把
`body=`（空串）原样回传，若把它拼进原文，验签会**永久失败**；
`python-alipay-sdk` 的 `AliPay.verify()` 不剔除空值，故不直接使用它。
验签失败时日志会打印参与签名的原文与空值参数名，便于定位。
