...
### 9.1 小程序目前无法在端内打开收银台 ⚠️

**这是当前唯一没打通的环节**，需要你知道：

- 后端现在实现的是**电脑网站支付**（`alipay.trade.page.pay`），面向 PC 浏览器。
- 微信小程序**不能用 `web-view` 打开外部链接**，除非该域名在小程序后台配置为
  「业务域名」（要求 HTTPS + 已备案 + 放校验文件）。
  `flightagent.nat100.top` 是内网穿透域名，能否通过校验不确定。
- 所以上面 5.1 的示例代码里，`openCashier` 回调用的是**复制链接 → 引导浏览器打开**
  的过渡方案，体验不完美但不阻塞联调。

**正确解法（后端待办）**：补一个**手机网站支付**（`alipay.trade.wap.pay`）渠道。
它会返回同样形态的表单，但在手机浏览器里是移动版收银台，且能自动唤起支付宝 App。
届时小程序可用 `web-view`（配好业务域名后）或安卓 WebView 直接打开，体验正常。
安卓端如果觉得 PC 版收银台排版难看，也等这个。

**需要你在接入前跟后端确认一句**：`wap.pay` 是否已上线。
