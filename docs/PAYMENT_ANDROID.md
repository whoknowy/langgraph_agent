...
> **退款（2026-09-16 起）**：退票退款已在服务端直连 `alipay.trade.refund` 原路退回，
> 客户端只需调 `/api/refund`（凭证来自 `/api/refund_quote`），**不要**自己调支付宝退款接口。
> 调用方式、参数与七种异常的处理约定见 `docs/PAYMENT_HANDOFF.md` 第 4.5 节。
...
**只看 `paid` 一个字段就够了**——你不用关心 `order_status` 具体是"已出票"还是"已改签"。

> ⚠️ **例外：改签补差价**。等差价期间订单状态本身就是「已出票」，此时后端会返回
> `"paid": false` 且带 `"change_pending": true`（`paid` 的含义变成"差价还没到账、
> 改签还没生效"）。所以**永远以 `paid` 为准，别用 `order_status` 反推支付结果**。
> 改签的完整流程见 `PAYMENT_HANDOFF.md` 第 4.6 节。
