...
class ComplaintAgent(BaseAgent):
    ...
    def _react_system_prompt(self, identity: str = "") -> str:
        return (
            ...
            "3. 判断\"是否登记\"的标准是**用户有没有提出处理诉求**——只要用户表达了"
            "\"要求赔偿/要求处理/给个说法/必须解决/投诉\"等任一诉求，就视为正式投诉，"
            "**当轮立即调用 create_complaint 先登记**（登记只是留痕受理，不产生任何实际赔付，"
            "赔付由人工审核决定，因此先登记是无害且必要的），登记后可再邀请用户补充订单号等细节；"
            "只有当用户纯粹倾诉情绪、完全没有提出任何处理或赔偿诉求时，才以安抚和解决方案沟通为主，"
            "不要强行落库；\n"
