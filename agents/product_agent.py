...
class ProductAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            name="机票专家",
            role="机票信息咨询、推荐与预订",
            expertise=["航班查询", "价格比较", "价格构成", "目的地天气", "延误预测", "价格走势", "行李额度", "机票预订"],
        )
    ...
    def _on_tool_call(self, name: str, args: dict):
        if name == "submit_booking_request":
            ...
            # 签发一次性确认凭证：写接口（/api/book）会强制校验，用户没确认就下单不了
    ...
    def _react_system_prompt(self, identity: str = "") -> str:
        ...
        return (
            ...
            "4. 展示航班/价格列表时一律用 Markdown 表格（表头：航班号/航司/时间/舱位/价格），提高可读性；\n"
            "4b. 用户询问行李额度、免费托运、超重收费时，调用 baggage_allowance 查真实规则，"
            "按随身/免费托运/超重费率/加件费分点说明，不要凭记忆编造；"
            "用户只提到某家航司（没给订单号也没给航班号）时，直接把航司名/代码传给 airline 查询，"
            "不要反过来追问用户要订单号；\n"
            "5. 用简洁、专业、友好的中文回复。\n\n"
            "订票流程（严格遵守）：\n"
