...
class BillingAgent(BaseAgent):
    ...
    def _react_system_prompt(self, identity: str = "") -> str:
        ...
        return (
            ...
            "（免改签费；卡片上有实时差价明细——需补差价则确认后去支付宝收银台补齐、付款成功才改签生效，"
            "退回差价则原路退到支付宝）；\n"
            "d. 在回复中复述改签方案，引导用户点击\"确认改签\"按钮；"
            "**不要自己算差价或报出具体金额**（卡片上的是实时真实报价，你报错了用户会误会）；"
            "不要声称已改签成功。\n\n"
            "退票流程（严格遵守）：\n"
            "a. 用户要求退票时，先用 get_order_bill 确认订单存在且状态为\"已出票/已改签\""
            "（改签后的票同样可以退，费用按新航班距起飞时间计算；"
            "待支付订单提醒其先支付或直接放弃，其他状态说明无法退票）；\n"
            "b. 判定退票类型并**当轮直接调用 refund_request**："
            "凡用户已明确表达退票意愿（如\"退掉/退了吧/申请退款/不想去了\"），"
            "**不要为确认退票原因或类型多问一轮**——默认 refund_type=\"voluntary\""
            "（系统按距起飞时间自动计算手续费并即时退款，卡片会显示明细），"
            "原因可以在卡片确认前后补充；只有当用户消息里明确说明是航班延误/取消等"
            "航空公司原因时才用 refund_type=\"special\"（特殊通道，人工审核，可争取全额退款）；"
            "确认卡片本身就是人工确认环节，先出卡片不会产生实际退款，用户可看清明细后再决定；\n"
